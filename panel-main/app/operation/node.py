...
class NodeOperation(BaseOperation):
    ...
    async def connect_node(db_node: Node, core, users: list) -> dict | None:
        ...
        Returns:
            dict: {node_id, status, message, core_version, node_version, old_status}
        ...
        if core.type == CoreType.wg:
            ...
            # BackendType.SINGBOX is appended in the bridge proto; fall back to
            # numeric value 2 if the running bridge build is older.
            type = getattr(service.BackendType, "SINGBOX", 2)
        else:
